	...
	bool PhysicsEngine::Update(float dt)
	{
		for (int i = 0; i < m_boxColliders.Count() - 1; i++)
		{
			for (int k = i + 1; k < m_boxColliders.Count(); k++)
			{
				...
				Vector2 l1(currentPosition->x + m_boxColliders[i]->rect.x, currentPosition->y + m_boxColliders[i]->rect.y);
				Vector2 r1(l1.x + m_boxColliders[i]->rect.width, l1.y + m_boxColliders[i]->rect.height);
				Vector2* otherPosition = m_boxColliders[k]->owner->transform->position;
				Vector2 l2(otherPosition->x + m_boxColliders[k]->rect.x, otherPosition->y + m_boxColliders[k]->rect.y);
				Vector2 r2(l2.x + m_boxColliders[k]->rect.width, l2.y + m_boxColliders[k]->rect.height);
